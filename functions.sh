...
function post {
    ...
    resp=$(curl "192.168.2.132:8123/api/services/device_tracker/see" -sfSX POST \
        -H 'Content-Type: application/json' \
        -H "X-HA-Access: XXXXXXXX" \
...
function get_ip {
    # get ip for mac
    grep "$1" /proc/net/arp | cut -f 1 -s -d" "
}

function get_host_name {
    # get hostname for mac
    nslookup "$(get_ip $1)" | grep .lan | cut -d ' ' -f 4 | cut -d '.' -f 1
}
...
function sync_state2() {
    for interface in `iw dev | grep Interface | cut -f 2 -s -d" "`; do
        maclist=`iw dev $interface station dump | grep Station | cut -f 2 -s -d" "`
        for mac in $maclist; do		    
			echo "$(get_host_name $mac)"
            post $(build_payload "$mac" "$(get_host_name $mac)" "300")
        done
    done
}
